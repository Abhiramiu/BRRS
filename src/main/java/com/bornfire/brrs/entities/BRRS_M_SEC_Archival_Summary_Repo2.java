...
public interface BRRS_M_SEC_Archival_Summary_Repo2
        extends JpaRepository<BRRS_M_SEC_Archival_Summary_Entity2,M_SEC_PK> {

        	 @Query(value = "select REPORT_DATE, REPORT_VERSION from BRRS_M_SEC_ARCHIVALTABLE_SUMMARY2 order by REPORT_VERSION", nativeQuery = true)
        	    ...
        	    List<BRRS_M_SEC_Archival_Summary_Entity2> getdatabydateListarchival(Date report_date, BigDecimal report_version);
        	    ...
        	    @Query("SELECT MAX(e.report_version) FROM BRRS_M_SEC_Archival_Summary_Entity2 e WHERE e.report_date = :date")
