...
public interface BrrsMNosvosP4ArchivalRepository extends JpaRepository<BrrsMNosvosP4Archival, BRRS_NOSVOS_Summary_PK> {
	
	@Query(value = "SELECT * FROM BRRS_M_NOSVOS_P4_ARCHIVAL WHERE TRUNC(REPORT_DATE) = TRUNC(?1)", nativeQuery = true)
	List<BrrsMNosvosP4Archival> getDataByDate(Date report_date);
