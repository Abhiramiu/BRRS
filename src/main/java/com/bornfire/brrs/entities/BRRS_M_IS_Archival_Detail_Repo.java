...
public interface BRRS_M_IS_Archival_Detail_Repo extends JpaRepository<M_IS_Archival_Detail_Entity, String> {
	...
	@Query(value = "select * from BRRS_M_IS_ARCHIVALTABLE_DETAIL where ROW_ID =?1 and COLUMN_ID=?2 AND REPORT_DATE=?3 AND DATA_ENTRY_VERSION=?4", nativeQuery = true)
	List<M_IS_Archival_Detail_Entity> GetDataByRowIdAndColumnId(String rowId,String ColumnId,Date reportdate,String DATA_ENTRY_VERSION);
}