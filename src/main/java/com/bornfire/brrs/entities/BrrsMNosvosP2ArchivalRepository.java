...
public interface BrrsMNosvosP2ArchivalRepository extends JpaRepository<BrrsMNosvosP2Archival, BRRS_NOSVOS_Summary_PK> {
    ...
    @Query(value = "select * from BRRS_M_NOSVOS_P2_ARCHIVAL where REPORT_DATE = ?1 and REPORT_VERSION = ?2", nativeQuery = true)
    List<BrrsMNosvosP2Archival> getdatabydateListarchival(Date report_date, BigDecimal report_version);
