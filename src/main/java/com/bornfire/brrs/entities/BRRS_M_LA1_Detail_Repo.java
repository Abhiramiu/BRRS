...
public interface BRRS_M_LA1_Detail_Repo extends JpaRepository<M_LA1_Detail_Entity, Date> {
	...
	@Query(value = "select * from BRRS_M_LA1_DETAILTABLE where report_label =?1 and report_addl_criteria_1=?2 AND REPORT_DATE=?3", nativeQuery = true)
	List<M_LA1_Detail_Entity> GetDataByRowIdAndColumnId(String rowId,String ColumnId,Date reportdate);
}
