...
public interface BRRS_M_IS_Detail_Repo extends JpaRepository<M_IS_Detail_Entity, String> {
	...
	@Query(value = "select * from BRRS_M_IS_DETAILTABLE WHERE REPORT_DATE =?1 and COLUMN_ID =?2 and ROW_ID=?3 ", nativeQuery = true)
	List<M_IS_Detail_Entity> GetDataByRowIdAndColumnId(String rowId,String ColumnId,Date reportdate);
}
