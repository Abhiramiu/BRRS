...
@Repository
public interface M_LA2_Detail_Repo extends JpaRepository<M_LA2_Detail_Entity,String> {
    ...
    List<M_LA2_Detail_Entity> getdatabydateList(Date reportdate, int offset, int limit);
 ...
 // Count rows by date
    ...
    @Query(value ="select * from BRRS_M_LA2_DETAILTABLE where REPORT_LABLE =?1 and REPORT_ADDL_CRITERIA_1=?2 AND REPORT_DATE=?3"
    		  , nativeQuery = true) 
    List<M_LA2_Detail_Entity> GetDataByRowIdAndColumnId(String reportLable,String reportAddlCriteria_1,Date reportdate);
