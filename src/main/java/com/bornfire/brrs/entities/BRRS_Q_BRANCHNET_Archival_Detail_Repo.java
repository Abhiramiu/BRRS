...
public interface BRRS_Q_BRANCHNET_Archival_Detail_Repo extends JpaRepository<Q_BRANCHNET_Archival_Detail_Entity, Date> {
	...
	@Query(value = "select * from BRRS_Q_BRANCHNET_ARCHIVALTABLE_DETAIL where REPORT_DATE = ?1 and REPORT_VERSION = ?2", nativeQuery = true)
	List<Q_BRANCHNET_Archival_Detail_Entity> getdatabydateListarchival(Date report_date, BigDecimal report_version);
