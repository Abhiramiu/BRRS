...
public interface BRRS_M_SRWA_12A_Detail_Repo extends JpaRepository<M_SRWA_12A_Detail_Entity, String> {
	
	
	// Fetch all records for a given date
	   @Query(value = "select * from BRRS_M_SRWA_12A_DETAILTABLE where REPORT_DATE = ?1", nativeQuery = true)
	   List<M_SRWA_12A_Detail_Entity> getdatabydateList(Date reportdate);
	   
	   // ✅ Pagination fixed → use OFFSET and LIMIT correctly
	   @Query(value = "select * from BRRS_M_SRWA_12A_DETAILTABLE where REPORT_DATE = ?1 offset ?2 rows fetch next ?3 rows only", nativeQuery = true)
	   List<M_SRWA_12A_Detail_Entity> getdatabydateList(Date reportdate, int offset, int limit);
	   
	   // Count rows by date
	   @Query(value = "select count(*) from BRRS_M_SRWA_12A_DETAILTABLE where REPORT_DATE = ?1", nativeQuery = true)
