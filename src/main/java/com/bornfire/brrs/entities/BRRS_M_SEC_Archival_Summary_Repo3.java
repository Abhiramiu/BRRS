...
public interface BRRS_M_SEC_Archival_Summary_Repo3 
    ...
    @Query(value = "SELECT * FROM BRRS_M_SEC_ARCHIVALTABLE_SUMMARY3 " +"WHERE REPORT_DATE = ?1 AND REPORT_VERSION IS NOT NULL " + 
    ...
    @Query(value = "SELECT *  FROM BRRS_M_SEC_ARCHIVALTABLE_SUMMARY3 WHERE REPORT_VERSION IS NOT NULL ORDER BY REPORT_VERSION DESC FETCH FIRST 1 ROWS ONLY ", nativeQuery = true)
    List<BRRS_M_SEC_Archival_Summary_Entity1> getdatabydateListWithVersion();

    @Query(value = "SELECT *  FROM BRRS_M_SEC_ARCHIVALTABLE_SUMMARY3 WHERE REPORT_VERSION IS NOT NULL ", nativeQuery = true)
