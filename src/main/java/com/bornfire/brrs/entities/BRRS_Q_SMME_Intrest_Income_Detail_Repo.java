...
public interface BRRS_Q_SMME_Intrest_Income_Detail_Repo extends JpaRepository<Q_SMME_Intrest_Income_Detail_Entity, String> {
  // Fetch all records for a given date
   @Query(value = "select * from BRRS_Q_SMME_INTREST_INCOME_DETAILTABLE where REPORT_DATE = ?1", nativeQuery = true)
   List<Q_SMME_Intrest_Income_Detail_Entity> getdatabydateList(Date parsedDate);
   // ✅ Pagination fixed → use OFFSET and LIMIT correctly
   @Query(value = "select * from BRRS_Q_SMME_INTREST_INCOME_DETAILTABLE where REPORT_DATE = ?1 offset ?2 rows fetch next ?3 rows only", nativeQuery = true)
   List<Q_SMME_Intrest_Income_Detail_Entity> getdatabydateList(Date reportdate, int offset, int limit);
   // Count rows by date
   @Query(value = "select count(*) from BRRS_Q_SMME_INTREST_INCOME_DETAILTABLE where REPORT_DATE = ?1", nativeQuery = true)
