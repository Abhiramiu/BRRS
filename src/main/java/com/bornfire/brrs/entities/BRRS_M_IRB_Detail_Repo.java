...
public interface BRRS_M_IRB_Detail_Repo extends JpaRepository<BRRS_M_IRB_Detail_Entity, String> {
    @Query(value = "select * from M_IRB_DETAIL_TABLE where REPORT_DATE =?1  ", nativeQuery = true)
    List<BRRS_M_IRB_Detail_Entity> getdatabydateList(Date reportdate);

    @Query(value = "select * from M_IRB_DETAIL_TABLE where ROW_ID =?1 and COLUMN_ID=?2 AND REPORT_DATE=?3", nativeQuery = true)
    List<BRRS_M_IRB_Detail_Entity> GetDataByRowIdAndColumnId(String rowId,String ColumnId,Date reportdate);

    @Query(value = "select * from M_IRB_DETAIL_TABLE where REPORT_DATE=?1 offset ?2 rows fetch next ?3 rows only", nativeQuery = true)
    List<BRRS_M_IRB_Detail_Entity> getdatabydateList(Date reportdate,int startpage,int endpage);
