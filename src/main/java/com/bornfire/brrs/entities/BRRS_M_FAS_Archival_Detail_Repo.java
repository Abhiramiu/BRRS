...
public interface BRRS_M_FAS_Archival_Detail_Repo extends JpaRepository<M_FAS_Archival_Detail_Entity, String> {
	...
	@Query(value = "select * from BRRS_M_FAS_ARCHIVALTABLE_DETAIL where REPORT_LABEL =?1 and REPORT_ADDL_CRITERIA_1=?2 AND to_date(?3,'dd-MM-yyyy') AND DATA_ENTRY_VERSION=?4", nativeQuery = true)
