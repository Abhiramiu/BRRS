...
public interface BRRS_M_CA5_RESUB_Detail_Repo2 extends JpaRepository<M_CA5_RESUB_Detail_Entity2 , M_CA5_Archival_Detail2_PK> {
	 ...
	 @Query(value = "select REPORT_DATE, REPORT_VERSION from BRRS_M_CA5_RESUB_DETAILTABLE2 order by REPORT_VERSION", nativeQuery = true)
	    ...
	    @Query("SELECT MAX(e.reportVersion) FROM M_CA5_RESUB_Detail_Entity2 e WHERE e.reportDate = :date")
		BigDecimal findMaxVersion(@Param("date") Date date);


		 //Current Report Version Only Shown 
	    @Query(value = "SELECT *  FROM BRRS_M_CA5_RESUB_DETAILTABLE2 WHERE REPORT_VERSION IS NOT NULL ORDER BY REPORT_VERSION ASC FETCH FIRST 1 ROWS ONLY ", nativeQuery = true)
	    List<M_CA5_RESUB_Detail_Entity2> getdatabydateListWithVersion();
