...
public interface BRRS_M_INT_RATES_Archival_Summary_Repo 
        extends JpaRepository<M_INT_RATES_Archival_Summary_Entity, M_INT_RATES_PK> {
    ...
    @Query(value = "SELECT * FROM BRRS_M_INT_RATES_ARCHIVALTABLE_SUMMARY WHERE REPORT_VERSION IS NOT NULL ORDER BY REPORT_VERSION ASC", nativeQuery = true)
    List<M_INT_RATES_Archival_Summary_Entity> getdatabydateListWithVersion();
    
    @Query("SELECT MAX(e.report_version) FROM M_INT_RATES_Archival_Summary_Entity e WHERE e.report_date = :date")
