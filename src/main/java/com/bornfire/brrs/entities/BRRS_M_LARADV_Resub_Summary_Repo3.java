...
public interface BRRS_M_LARADV_Resub_Summary_Repo3  extends JpaRepository<M_LARADV_Resub_Summary_Entity3,M_LARADV_PK>{
    ...
    @Query("SELECT e FROM M_LARADV_Resub_Summary_Entity3 e " + "WHERE e.report_date = :reportDate "
	...
	@Query(value = "SELECT NVL(MAX(report_version), 0) FROM BRRS_M_LARADV_RESUBTABLE_SUMMARY3", nativeQuery = true)
