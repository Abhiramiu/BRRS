...
@Repository
public interface BRRS_M_BOP_Archival_Detail_Repo extends JpaRepository<M_BOP_Archival_Detail_Entity, M_BOP_PK>{
	    ...
	    @Query("SELECT MAX(e.report_version) " +
	    	       "FROM M_BOP_Archival_Detail_Entity e " +
	    ...
	    @Query(value = "SELECT *  FROM BRRS_M_BOP_ARCHIVALTABLE_DETAIL WHERE REPORT_VERSION IS NOT NULL ORDER BY REPORT_VERSION ASC FETCH FIRST 1 ROWS ONLY ", nativeQuery = true)
	    List<M_BOP_Archival_Detail_Entity> getdatabydateListWithVersion();
