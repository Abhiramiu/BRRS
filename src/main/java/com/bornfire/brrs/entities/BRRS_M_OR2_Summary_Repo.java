...
public interface BRRS_M_OR2_Summary_Repo
    ...
    List<M_OR2_Summary_Entity> getdatabydateList(Date rpt_date);

    @Query(value = "SELECT *  FROM BRRS_M_OR2_SUMMARYTABLE WHERE REPORT_DATE = ?1   AND REPORT_VERSION IS NOT NULL ORDER BY REPORT_VERSION DESC FETCH FIRST 1 ROWS ONLY ", nativeQuery = true)
    List<M_OR2_Summary_Entity> getdatabydateListWithVersion(String todate);

    // Find the latest version for a report date
    Optional<M_OR2_Summary_Entity> findTopByReportDateOrderByReportVersionDesc(Date reportDate);
