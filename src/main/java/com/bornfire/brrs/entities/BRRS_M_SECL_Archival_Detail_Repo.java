...
public interface BRRS_M_SECL_Archival_Detail_Repo extends JpaRepository<M_SECL_Archival_Detail_Entity,Date>
{
    ...
    @Query(value = "SELECT * FROM BRRS_M_SECL_ARCHIVALTABLE_DETAIL " +
                   "WHERE REPORT_DATE = ?1 AND REPORT_VERSION =?2 ",
           nativeQuery = true)
    List<M_SECL_Archival_Detail_Entity> getdatabydateListarchival(
    ...
    @Query(value = "SELECT * FROM BRRS_M_SECL_ARCHIVALTABLE_DETAIL " +
           ...
           nativeQuery = true)
    Optional<M_SECL_Archival_Detail_Entity> getLatestArchivalVersionByDate(
            Date reportDate);

    // Fetch by primary key (FIXED incorrect return type)
    Optional<M_SECL_Archival_Detail_Entity> findByReportDateAndReportVersion(
    ...
    @Query(value = "SELECT * FROM BRRS_M_SECL_ARCHIVALTABLE_DETAIL " +
                   ...
                   "FETCH FIRST 1 ROWS ONLY",
           nativeQuery = true)
    List<M_SECL_Archival_Detail_Entity> getdatabydateListWithVersion();
    ...
    @Query(value = "SELECT * FROM BRRS_M_SECL_ARCHIVALTABLE_DETAIL " +
                   ...
                   "ORDER BY TO_NUMBER(REPORT_VERSION) ASC",
           nativeQuery = true)
    List<M_SECL_Archival_Detail_Entity> getdatabydateListWithVersionAll();
}
