...
public interface BRRS_MDISB3_Detail_Repo extends JpaRepository<MDISB3_Detail_Entity, String> {
	...
	@Query(value = "select * from BRRS_MDISB3_DETAILTABLE  where REPORT_LABEL =?1 and REPORT_ADDL_CRITERIA_1=?2 and REPORT_DATE=?3", nativeQuery = true)
	List<MDISB3_Detail_Entity> GetDataByRowIdAndColumnId(String report_label,String report_addl_criteria_1,Date reportdate);
	
	@Query(value = "select * from BRRS_MDISB3_DETAILTABLE where REPORT_DATE=?1 offset ?2 rows fetch next ?3 rows only", nativeQuery = true)
	List<MDISB3_Detail_Entity> getdatabydateList(Date reportdate,int startpage,int endpage);
	
	@Query(value = "select count(*) from BRRS_MDISB3_DETAILTABLE where REPORT_DATE=?1", nativeQuery = true)
