...
public interface BRRS_Q_RLFA1_Detail_Repo_New extends JpaRepository<Q_RLFA1_Detail_Entity_New, Date> {
	...
	@Query(value = "select * from BRRS_Q_RLFA1_DETAIL_TABLE where REPORT_fDATE =?1  ", nativeQuery = true)
	List<Q_RLFA1_Detail_Entity_New> getdatabydateList(Date reportdate);

	@Query(value = "SELECT *  FROM BRRS_Q_RLFA1_DETAIL_TABLE WHERE REPORT_DATE = ?1   AND REPORT_VERSION IS NOT NULL ORDER BY REPORT_VERSION DESC FETCH FIRST 1 ROWS ONLY ", nativeQuery = true)
	List<Q_RLFA1_Detail_Entity_New> getdatabydateListWithVersion(String todate);

	// ✅ Find the latest version for a report date
	@Query(value = "SELECT * FROM BRRS_Q_RLFA1_DETAIL_TABLE " + "WHERE REPORT_DATE = ?1 "
	...
	@Query(value = "SELECT * FROM BRRS_Q_RLFA1_DETAIL_TABLE "
	...
	@Query(value = "SELECT *  FROM BRRS_Q_RLFA1_DETAIL_TABLE WHERE REPORT_VERSION IS NOT NULL ORDER BY REPORT_VERSION DESC FETCH FIRST 1 ROWS ONLY ", nativeQuery = true)
