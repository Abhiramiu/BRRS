...
public interface BRRS_PL_SCHS_Manual_Archival_Summary_Repo extends JpaRepository<PL_SCHS_Manual_Archival_Summary_Entity, Date> {
    ...
    List<PL_SCHS_Manual_Archival_Summary_Entity> getdatabydateListarchival(Date report_date, BigDecimal report_version);
}
//         // Fetch specific archival data by report date & version
//     @Query(value = "SELECT * FROM BRRS_PL_SCHS_MANUAL_ARCHIVALTABLE_SUMMARY " +
//                    "WHERE REPORT_DATE = ?1 AND REPORT_VERSION = ?2",
//            nativeQuery = true)
//     List<PL_SCHS_Manual_Archival_Summary_Entity> getdatabydateListarchival(
