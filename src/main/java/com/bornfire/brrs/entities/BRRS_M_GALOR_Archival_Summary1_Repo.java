...
@Repository
public interface BRRS_M_GALOR_Archival_Summary1_Repo 
        extends JpaRepository<M_GALOR_Archival_Summary_Entity1, M_GALOR_PK> {

    // -------------------------------------------------------
    // 1️⃣  Archival View (Only Report Date & Version)
    // -------------------------------------------------------
    @Query(value = "SELECT REPORT_DATE, REPORT_VERSION " +
                   "FROM BRRS_M_GALOR_ARCHIVALTABLE_SUMMARY1 " +
    ...
    // -------------------------------------------------------
    @Query(value = "SELECT * FROM BRRS_M_GALOR_ARCHIVALTABLE_SUMMARY1 " +
                   "WHERE REPORT_DATE = ?1 " +
                   "AND REPORT_VERSION = ?2",
           nativeQuery = true)
    List<M_GALOR_Archival_Summary_Entity1> 
