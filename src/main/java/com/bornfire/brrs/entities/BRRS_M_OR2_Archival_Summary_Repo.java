...
public interface BRRS_M_OR2_Archival_Summary_Repo 
    ...
    @Query(value = "SELECT * FROM BRRS_M_OR2_ARCHIVALTABLE_SUMMARY " + "WHERE REPORT_DATE = ?1 AND REPORT_VERSION = ?2",nativeQuery = true)
    List<M_OR2_Archival_Summary_Entity> getdatabydateListarchival(Date reportDate, String reportVersion);

    //  Fetch latest archival version for given date (no version input)
    @Query(value = "SELECT * FROM BRRS_M_OR2_ARCHIVALTABLE_SUMMARY " +"WHERE REPORT_DATE = ?1 AND REPORT_VERSION IS NOT NULL " + 
    ...
    @Query(value = "SELECT *  FROM BRRS_M_OR2_ARCHIVALTABLE_SUMMARY WHERE REPORT_VERSION IS NOT NULL ORDER BY REPORT_VERSION DESC FETCH FIRST 1 ROWS ONLY ", nativeQuery = true)
    List<M_OR2_Archival_Summary_Entity> getdatabydateListWithVersion();

    @Query(value = "SELECT *  FROM BRRS_M_OR2_ARCHIVALTABLE_SUMMARY WHERE REPORT_VERSION IS NOT NULL ", nativeQuery = true)
