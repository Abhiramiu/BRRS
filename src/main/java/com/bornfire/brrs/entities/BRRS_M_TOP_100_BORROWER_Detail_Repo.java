...
public interface BRRS_M_TOP_100_BORROWER_Detail_Repo extends JpaRepository<M_TOP_100_BORROWER_Detail_Entity, String> {
   ...
   List<M_TOP_100_BORROWER_Detail_Entity> getdatabydateList(Date reportdate);
   
   // ✅ Pagination fixed → use OFFSET and LIMIT correctly
   @Query(value = "select * from BRRS_M_TOP_100_BORROWER_DETAILTABLE where REPORT_DATE = ?1 offset ?2 rows fetch next ?3 rows only", nativeQuery = true)
   List<M_TOP_100_BORROWER_Detail_Entity> getdatabydateList(Date reportdate, int offset, int limit);
   
   // Count rows by date
   @Query(value = "select count(*) from BRRS_M_TOP_100_BORROWER_DETAILTABLE where REPORT_DATE = ?1", nativeQuery = true)
   int getdatacount(Date reportdate);
   
	  @Query(value =
	  "select * from BRRS_M_TOP_100_BORROWER_DETAILTABLE where REPORT_LABLE =?1 and REPORT_ADDL_CRITERIA_1=?2 AND REPORT_DATE=?3"
