...
//				"ACTIVE");
	...
	@RequestMapping(value = "SLS", method = { RequestMethod.GET, RequestMethod.POST })
	public String SLS(Model md, HttpServletRequest req) {
		md.addAttribute("menu", "RBR Reports- SLS");
		System.out.println("count" + rrReportlist.getReportListbrrsSLS().size());
		md.addAttribute("reportlist", rrReportlist.getReportListbrrsSLS());
	...
	public String brrsHalfYearlyArchival(Model md, HttpServletRequest req) {
		...
		System.out.println("Half Yearly count: " + rrReportlist.getReportListbrrsH().size());
		md.addAttribute("reportlist", rrReportlist.getReportListbrrsH());
...
//	}
	...
	/*
	 * @RequestMapping(value = "SLSREPORT", method = { RequestMethod.GET,
	 * RequestMethod.POST }) public ModelAndView SLSREPORT(@RequestParam(required =
	 * false) String todate, // Key changed from fromdate/asondate // to todate
	 * 
	 * @RequestParam(required = false) String currency, @RequestParam(required =
	 ...
	 * 
	 * // Call service. formmode (summary/Detail) maps to dtltype return
	 * BRRS_SLS_INPUT_SHT_reportservice.getSLSView("SLS", null, todate, currency,
	 * formmode, pageable); }
	 */
	...
	public ResponseEntity<String> checkBFDBUpload(@RequestParam String jobId) {
		...
		// The service returns: "PROCESSING", "NOT_FOUND", "COMPLETED:..." or
		// "ERROR:..."
		if (status.startsWith("COMPLETED:") || status.startsWith("ERROR:")) {
	...
	public String CommonMapping(@RequestParam(required = false) String formmode,
		...
		if (formmode == null || formmode.equals("list")) {
			...
			System.out.println(
					formmode.substring(0, 1).toUpperCase() + formmode.substring(1) + " Account no is " + ACCOUNT_NO);
			md.addAttribute("formmode", formmode);
	...
	@RequestMapping(value = "QuarterlyResub", method = { RequestMethod.GET, RequestMethod.POST })
	public String brrsQuarterlyResub(Model md, HttpServletRequest req) {
		md.addAttribute("menu", "BRRS - BRRS QUARTERLY RESUBMISSION");
