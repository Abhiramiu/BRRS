...
@Service
@Component
public class BRRS_IRRBB_DEPOSITS_ReportService {

	private static final Logger logger = LoggerFactory.getLogger(BRRS_IRRBB_DEPOSITS_ReportService.class);
	...
	// Used ONLY when calling the stored procedures, which expect 'DD-MM-YYYY'
	SimpleDateFormat procDateFormat = new SimpleDateFormat("dd-MM-yyyy");
	...
	// Summary Entity
	public static class IRRBB_DEPOSITS_Summary_Entity {
		private Long sno;
	...
	// Detail Entity (identical field set to Summary)
	public static class IRRBB_DEPOSITS_Detail_Entity {
		private Long sno;
		private String customerId;
		private String accountNumber;
		private String glCode;
	...
	// Archival Summary Entity (Summary + REPORT_RESUBDATE)
	public static class IRRBB_DEPOSITS_Archival_Summary_Entity {
		private Long sno;
		private String customerId;
		private String accountNumber;
		private String glCode;
	...
	// Archival Detail Entity (identical shape to Archival Summary)
	public static class IRRBB_DEPOSITS_Archival_Detail_Entity {
		private Long sno;
		private String customerId;
		private String accountNumber;
		private String glCode;
	...
	private class SummaryRowMapper implements RowMapper<IRRBB_DEPOSITS_Summary_Entity> {
		@Override
		public IRRBB_DEPOSITS_Summary_Entity mapRow(ResultSet rs, int rowNum) throws SQLException {
			IRRBB_DEPOSITS_Summary_Entity entity = new IRRBB_DEPOSITS_Summary_Entity();
	...
	private class DetailRowMapper implements RowMapper<IRRBB_DEPOSITS_Detail_Entity> {
		@Override
		public IRRBB_DEPOSITS_Detail_Entity mapRow(ResultSet rs, int rowNum) throws SQLException {
			IRRBB_DEPOSITS_Detail_Entity entity = new IRRBB_DEPOSITS_Detail_Entity();
			entity.setSno(rs.getLong("SNO"));
			entity.setCustomerId(rs.getString("CUSTOMER_ID"));
			entity.setAccountNumber(rs.getString("ACCOUNT_NUMBER"));
			entity.setGlCode(rs.getString("GL_CODE"));
	...
	private class ArchivalSummaryRowMapper implements RowMapper<IRRBB_DEPOSITS_Archival_Summary_Entity> {
		@Override
		public IRRBB_DEPOSITS_Archival_Summary_Entity mapRow(ResultSet rs, int rowNum) throws SQLException {
			IRRBB_DEPOSITS_Archival_Summary_Entity entity = new IRRBB_DEPOSITS_Archival_Summary_Entity();
			entity.setSno(rs.getLong("SNO"));
			entity.setCustomerId(rs.getString("CUSTOMER_ID"));
			entity.setAccountNumber(rs.getString("ACCOUNT_NUMBER"));
			entity.setGlCode(rs.getString("GL_CODE"));
	...
	private class ArchivalDetailRowMapper implements RowMapper<IRRBB_DEPOSITS_Archival_Detail_Entity> {
		@Override
		public IRRBB_DEPOSITS_Archival_Detail_Entity mapRow(ResultSet rs, int rowNum) throws SQLException {
			IRRBB_DEPOSITS_Archival_Detail_Entity entity = new IRRBB_DEPOSITS_Archival_Detail_Entity();
			entity.setSno(rs.getLong("SNO"));
			entity.setCustomerId(rs.getString("CUSTOMER_ID"));
			entity.setAccountNumber(rs.getString("ACCOUNT_NUMBER"));
			entity.setGlCode(rs.getString("GL_CODE"));
	...
	public ModelAndView getBRRS_IRRBB_DEPOSITS_View(String reportId, String fromdate, String todate, String currency,
		...
		if ("ARCHIVAL".equals(type) && version != null) {
			try {
				...
				List<IRRBB_DEPOSITS_Archival_Summary_Entity> archivalSummary = getArchivalDataByDateAndVersion(dt,
						version);

				System.out.println("Archival Summary size = " + archivalSummary.size());

				if ("detail".equalsIgnoreCase(dtltype)) {
					List<IRRBB_DEPOSITS_Archival_Detail_Entity> archivalDetail = getArchivalDetailDataByDateAndVersion(
							dt, version);
		...
		// =====================================================
			...
			try {
				...
				List<IRRBB_DEPOSITS_Archival_Summary_Entity> resubSummary = getResubDataByDateAndVersion(dt, version);

				System.out.println("Resub Summary size = " + resubSummary.size());

				if ("detail".equalsIgnoreCase(dtltype)) {
					List<IRRBB_DEPOSITS_Archival_Detail_Entity> resubDetail = getResubDetailDataByDateAndVersion(dt,
							version);
		...
		// =====================================================
			...
			try {
				...
				List<IRRBB_DEPOSITS_Summary_Entity> normalSummary = jdbcTemplate.query(
						"SELECT * FROM BRRS_IRRBB_DEPOSITS_SUMMARYTABLE WHERE REPORT_DATE = TO_DATE(?, 'DD-MON-YYYY')",
						new Object[] { formattedDate }, new SummaryRowMapper());

				System.out.println("Normal Summary size = " + normalSummary.size());

				if ("detail".equalsIgnoreCase(dtltype)) {
					List<IRRBB_DEPOSITS_Detail_Entity> normalDetail = jdbcTemplate.query(
							"SELECT * FROM BRRS_IRRBB_DEPOSITS_DETAILTABLE WHERE REPORT_DATE = TO_DATE(?, 'DD-MON-YYYY')",
		...
		mv.setViewName("BRRS/IRRBB_DEPOSITS");
	...
	// ===========================================================
	private void saveToArchivalFromNormal(IRRBB_DEPOSITS_Summary_Entity oldRecord, BigDecimal version) {
		String archivalSummarySql = "INSERT INTO BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE "
				+ "(SNO, CUSTOMER_ID, ACCOUNT_NUMBER, GL_CODE, GL_DESCRIPTION, ACCOUNT_CURRENCY, "
				+ "ACCOUNT_OPENING_DATE, MATURITY_DATE, OUTSTANDING_BALANCE_ACCT_CCY, OUTSTANDING_BALANCE_INR, "
				+ "FLOATING_FIXED, EXISTING_BENCHMARK, EXISTING_REPRICING_FREQUENCY, LAST_REPRICING_DATE, "
				+ "NEXT_REPRICING_DATE, SPREAD_OVER_BENCHMARK, FINAL_ROI_PERCENT, CAP_FLOOR_RATE_OF_INTEREST, "
				+ "REMARKS, BACID, REPORT_DATE, REPORT_VERSION, REPORT_FREQUENCY, REPORT_CODE, REPORT_DESC, "
				+ "ENTITY_FLG, MODIFY_FLG, DEL_FLG) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

		try {
			jdbcTemplate.update(archivalSummarySql, oldRecord.getSno(), oldRecord.getCustomerId(),
					oldRecord.getAccountNumber(), oldRecord.getGlCode(), oldRecord.getGlDescription(),
					oldRecord.getAccountCurrency(), oldRecord.getAccountOpeningDate(), oldRecord.getMaturityDate(),
					...
					oldRecord.getNextRepricingDate(), oldRecord.getSpreadOverBenchmark(),
					oldRecord.getFinalRoiPercent(), oldRecord.getCapFloorRateOfInterest(), oldRecord.getRemarks(),
					oldRecord.getBacid(), oldRecord.getReportDate(), version, oldRecord.getReportFrequency(),
					oldRecord.getReportCode(), oldRecord.getReportDesc(), oldRecord.getEntityFlg(),
					oldRecord.getModifyFlg(), oldRecord.getDelFlg());
		} catch (Exception e) {
			logger.error("Error saving to ARCHIVAL SUMMARY: {}", e.getMessage());
		}

		String archivalDetailSql = "INSERT INTO BRRS_IRRBB_DEPOSITS_ARCHIVAL_DETAILTABLE "
				+ "(SNO, CUSTOMER_ID, ACCOUNT_NUMBER, GL_CODE, GL_DESCRIPTION, ACCOUNT_CURRENCY, "
				+ "ACCOUNT_OPENING_DATE, MATURITY_DATE, OUTSTANDING_BALANCE_ACCT_CCY, OUTSTANDING_BALANCE_INR, "
				+ "FLOATING_FIXED, EXISTING_BENCHMARK, EXISTING_REPRICING_FREQUENCY, LAST_REPRICING_DATE, "
				+ "NEXT_REPRICING_DATE, SPREAD_OVER_BENCHMARK, FINAL_ROI_PERCENT, CAP_FLOOR_RATE_OF_INTEREST, "
	...
	private BigDecimal getNextArchivalVersion(String reportDate) {
		try {
			String sql = "SELECT COALESCE(MAX(REPORT_VERSION), 0) + 1 FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE WHERE REPORT_DATE = TO_DATE(?, 'DD-MON-YYYY')";
	...
	public void saveResubIrrdep(IRRBB_DEPOSITS_Archival_Summary_Entity summary) {
		Long nextSno = jdbcTemplate
				.queryForObject("SELECT BRRS_IRRBB_DEP_ARCHIVAL_SUMMARYTABLE_SNO_SEQ.NEXTVAL FROM DUAL", Long.class);
		summary.setSno(nextSno);

		String archivalSummarySql = "INSERT INTO BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE (SNO, CUSTOMER_ID, ACCOUNT_NUMBER, "
				+ "GL_CODE, GL_DESCRIPTION, ACCOUNT_CURRENCY, ACCOUNT_OPENING_DATE, MATURITY_DATE, "
				+ "OUTSTANDING_BALANCE_ACCT_CCY, OUTSTANDING_BALANCE_INR, FLOATING_FIXED, EXISTING_BENCHMARK, "
				+ "EXISTING_REPRICING_FREQUENCY, LAST_REPRICING_DATE, NEXT_REPRICING_DATE, SPREAD_OVER_BENCHMARK, "
				+ "FINAL_ROI_PERCENT, CAP_FLOOR_RATE_OF_INTEREST, REMARKS, BACID, REPORT_DATE, REPORT_VERSION, "
		...
		jdbcTemplate.update(archivalSummarySql, summary.getSno(), summary.getCustomerId(), summary.getAccountNumber(),
				summary.getGlCode(), summary.getGlDescription(), summary.getAccountCurrency(),
				summary.getAccountOpeningDate(), summary.getMaturityDate(), summary.getOutstandingBalanceAcctCcy(),
				summary.getOutstandingBalanceInr(), summary.getFloatingFixed(), summary.getExistingBenchmark(),
				summary.getExistingRepricingFrequency(), summary.getLastRepricingDate(), summary.getNextRepricingDate(),
				summary.getSpreadOverBenchmark(), summary.getFinalRoiPercent(), summary.getCapFloorRateOfInterest(),
				summary.getRemarks(), summary.getBacid(), summary.getReportDate(), summary.getReportVersion(),
		...
		String archivalDetailSql = "INSERT INTO BRRS_IRRBB_DEPOSITS_ARCHIVAL_DETAILTABLE (SNO, CUSTOMER_ID, ACCOUNT_NUMBER, "
				+ "GL_CODE, GL_DESCRIPTION, ACCOUNT_CURRENCY, ACCOUNT_OPENING_DATE, MATURITY_DATE, "
				+ "OUTSTANDING_BALANCE_ACCT_CCY, OUTSTANDING_BALANCE_INR, FLOATING_FIXED, EXISTING_BENCHMARK, "
				+ "EXISTING_REPRICING_FREQUENCY, LAST_REPRICING_DATE, NEXT_REPRICING_DATE, SPREAD_OVER_BENCHMARK, "
				+ "FINAL_ROI_PERCENT, CAP_FLOOR_RATE_OF_INTEREST, REMARKS, BACID, REPORT_DATE, REPORT_VERSION, "
	...
	public void saveResubIrrdep(IRRBB_DEPOSITS_Summary_Entity summary) {
		BigDecimal nextSno = jdbcTemplate.queryForObject(
				"SELECT BRRS_IRRBB_DEP_ARCHIVAL_SUMMARYTABLE_SNO_SEQ.NEXTVAL FROM DUAL", BigDecimal.class);
		IRRBB_DEPOSITS_Archival_Summary_Entity resubEntity = new IRRBB_DEPOSITS_Archival_Summary_Entity();
		if (nextSno != null) {
		...
		resubEntity.setFinalRoiPercent(summary.getFinalRoiPercent());
		resubEntity.setCapFloorRateOfInterest(summary.getCapFloorRateOfInterest());
		resubEntity.setRemarks(summary.getRemarks());
		resubEntity.setBacid(summary.getBacid());
		...
		saveResubIrrdep(resubEntity);
	}

	// Helper: Save resub data as new version in ARCHIVAL tables
	private void saveResubAsNewVersion(IRRBB_DEPOSITS_Archival_Summary_Entity oldRecord, BigDecimal newVersion) {
		Long nextSno = jdbcTemplate
				.queryForObject("SELECT BRRS_IRRBB_DEP_ARCHIVAL_SUMMARYTABLE_SNO_SEQ.NEXTVAL FROM DUAL", Long.class);
		...
		jdbcTemplate.update(archivalSummarySql, nextSno, oldRecord.getCustomerId(), oldRecord.getAccountNumber(),
				oldRecord.getGlCode(), oldRecord.getGlDescription(), oldRecord.getAccountCurrency(),
				oldRecord.getAccountOpeningDate(), oldRecord.getMaturityDate(),
				oldRecord.getOutstandingBalanceAcctCcy(), oldRecord.getOutstandingBalanceInr(),
				oldRecord.getFloatingFixed(), oldRecord.getExistingBenchmark(),
				oldRecord.getExistingRepricingFrequency(), oldRecord.getLastRepricingDate(),
				oldRecord.getNextRepricingDate(), oldRecord.getSpreadOverBenchmark(), oldRecord.getFinalRoiPercent(),
				oldRecord.getCapFloorRateOfInterest(), oldRecord.getRemarks(), oldRecord.getBacid(),
				oldRecord.getReportDate(), newVersion, oldRecord.getReportFrequency(), oldRecord.getReportCode(),
				oldRecord.getReportDesc(), oldRecord.getEntityFlg(), oldRecord.getModifyFlg(), oldRecord.getDelFlg(),
	...
	public List<IRRBB_DEPOSITS_Archival_Summary_Entity> getArchivalData(String reportDate, BigDecimal version) {
		String sql = "SELECT * FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE WHERE REPORT_DATE = TO_DATE(?, 'DD-MON-YYYY')";
		if (version != null) {
	...
	public List<IRRBB_DEPOSITS_Archival_Detail_Entity> getArchivalDetailData(String reportDate, BigDecimal version) {
		String sql = "SELECT * FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_DETAILTABLE WHERE REPORT_DATE = TO_DATE(?, 'DD-MON-YYYY')";
		if (version != null) {
			sql += " AND REPORT_VERSION = ?";
	...
	public List<IRRBB_DEPOSITS_Archival_Summary_Entity> getResubData(String reportDate, BigDecimal version) {
		String sql = "SELECT * FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE WHERE REPORT_DATE = TO_DATE(?, 'DD-MON-YYYY')";
		if (version != null) {
			sql += " AND REPORT_VERSION = ?";
			return jdbcTemplate.query(sql, new Object[] { reportDate, version }, new ArchivalSummaryRowMapper());
	...
	public List<IRRBB_DEPOSITS_Archival_Detail_Entity> getResubDetailData(String reportDate, BigDecimal version) {
		String sql = "SELECT * FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_DETAILTABLE WHERE REPORT_DATE = TO_DATE(?, 'DD-MON-YYYY')";
		if (version != null) {
			sql += " AND REPORT_VERSION = ?";
			return jdbcTemplate.query(sql, new Object[] { reportDate, version }, new ArchivalDetailRowMapper());
	...
	public List<BigDecimal> getArchivalVersions(String reportDate) {
		String sql = "SELECT DISTINCT REPORT_VERSION FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE WHERE REPORT_DATE = TO_DATE(?, 'DD-MON-YYYY') ORDER BY REPORT_VERSION DESC";
	...
	// ===========================================================
	public List<Object[]> getIRRBB_DEPOSITSArchival() {
		String sql = "SELECT DISTINCT REPORT_DATE, REPORT_VERSION, REPORT_RESUBDATE "
				+ "FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE " + "WHERE REPORT_VERSION IS NOT NULL "
	...
	// ===========================================================
	public List<IRRBB_DEPOSITS_Archival_Summary_Entity> getArchivalDataByDateAndVersion(Date reportDate,
			BigDecimal reportVersion) {

		String sql = "SELECT * FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE "
	...
	// ===========================================================
	public List<IRRBB_DEPOSITS_Archival_Detail_Entity> getArchivalDetailDataByDateAndVersion(Date reportDate,
			BigDecimal reportVersion) {

		String sql = "SELECT * FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_DETAILTABLE "
				+ "WHERE REPORT_DATE = ? AND REPORT_VERSION = ?";
	...
	// ===========================================================
	public List<IRRBB_DEPOSITS_Archival_Summary_Entity> getResubDataByDateAndVersion(Date reportDate,
			BigDecimal reportVersion) {
	...
	// ===========================================================
	public List<IRRBB_DEPOSITS_Archival_Detail_Entity> getResubDetailDataByDateAndVersion(Date reportDate,
			BigDecimal reportVersion) {
	...
	// ===========================================================
	public List<IRRBB_DEPOSITS_Archival_Summary_Entity> getAllArchivalWithVersion() {

		String sql = "SELECT * FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE "
				+ "WHERE SNO IN (SELECT MIN(SNO) FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE WHERE REPORT_VERSION IS NOT NULL GROUP BY REPORT_DATE, REPORT_VERSION) "
				+ "ORDER BY REPORT_DATE DESC, REPORT_VERSION DESC";
	...
	// ===========================================================
	public List<IRRBB_DEPOSITS_Archival_Summary_Entity> getAllResubWithVersion() {
	...
	public BigDecimal findMaxArchivalVersion(Date reportDate) {
		...
		String sql = "SELECT MAX(REPORT_VERSION) " + "FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE "
				+ "WHERE REPORT_DATE = ?";
	...
	// ===========================================================
	public List<Object[]> getIRRBB_DEPOSITSArchivalList() {

		String sql = "SELECT DISTINCT REPORT_DATE, REPORT_VERSION " + "FROM BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE "
	...
	// ===========================================================
	public List<Object[]> getIRRBB_DEPOSITSResubList() {
	...
	public byte[] getBRRS_IRRBB_DEPOSITS_Excel(String filename, String reportId, String fromdate, String todate,
		...
		if ("ARCHIVAL".equalsIgnoreCase(type) && version != null) {
			List<IRRBB_DEPOSITS_Archival_Summary_Entity> dataList = getArchivalData(formattedDate, version);
			return generateExcelFromArchivalData(dataList, filename, type);
		} else if ("RESUB".equalsIgnoreCase(type) && version != null) {
			List<IRRBB_DEPOSITS_Archival_Summary_Entity> dataList = getResubData(formattedDate, version);
			return generateExcelFromResubData(dataList, filename, type);
		} else {
			List<IRRBB_DEPOSITS_Summary_Entity> dataList = jdbcTemplate.query(
					"SELECT * FROM BRRS_IRRBB_DEPOSITS_SUMMARYTABLE WHERE REPORT_DATE = TO_DATE(?, 'DD-MON-YYYY')",
					new Object[] { formattedDate }, new SummaryRowMapper());
	...
	private byte[] generateExcelFromNormalData(List<IRRBB_DEPOSITS_Summary_Entity> dataList, String filename)
			throws Exception {
		if (dataList == null || dataList.isEmpty()) {
			logger.warn("No data found for IRRBB_DEPOSITS report.");
			return new byte[0];
		}
		...
		try (InputStream templateInputStream = Files.newInputStream(templatePath);
			...
			for (IRRBB_DEPOSITS_Summary_Entity data : dataList) {
				Row row = sheet.createRow(rowIndex++);
				setCellValue(row, 0, data.getCustomerId(), textStyle);
				setCellValue(row, 1, data.getAccountNumber(), textStyle);
				setCellValue(row, 2, data.getGlCode(), textStyle);
				setCellValue(row, 3, data.getGlDescription(), textStyle);
				setCellValue(row, 4, data.getAccountCurrency(), textStyle);
				setCellValue(row, 5, data.getAccountOpeningDate(), dateStyle);
				setCellValue(row, 6, data.getMaturityDate(), dateStyle);
				setCellValue(row, 7, data.getOutstandingBalanceAcctCcy(), numberStyle);
				setCellValue(row, 8, data.getOutstandingBalanceInr(), numberStyle);
				setCellValue(row, 9, data.getFloatingFixed(), textStyle);
				setCellValue(row, 10, data.getExistingBenchmark(), textStyle);
				setCellValue(row, 11, data.getExistingRepricingFrequency(), textStyle);
				setCellValue(row, 12, data.getLastRepricingDate(), dateStyle);
				setCellValue(row, 13, data.getNextRepricingDate(), dateStyle);
				setCellValue(row, 14, data.getSpreadOverBenchmark(), textStyle);
				setCellValue(row, 15, data.getFinalRoiPercent(), textStyle);
				setCellValue(row, 16, data.getCapFloorRateOfInterest(), textStyle);
				setCellValue(row, 17, data.getRemarks(), textStyle);
				setCellValue(row, 18, data.getBacid(), textStyle);
			}
	...
	private byte[] generateExcelFromArchivalData(List<IRRBB_DEPOSITS_Archival_Summary_Entity> dataList, String filename,
			String type) throws Exception {
		if (dataList == null || dataList.isEmpty()) {
			logger.warn("No archival data found for IRRBB_DEPOSITS report.");
			return new byte[0];
		}
		...
		try (InputStream templateInputStream = Files.newInputStream(templatePath);
			...
			for (IRRBB_DEPOSITS_Archival_Summary_Entity data : dataList) {
				Row row = sheet.createRow(rowIndex++);
				setCellValue(row, 0, data.getCustomerId(), textStyle);
				setCellValue(row, 1, data.getAccountNumber(), textStyle);
				setCellValue(row, 2, data.getGlCode(), textStyle);
				...
				setCellValue(row, 17, data.getRemarks(), textStyle);
				setCellValue(row, 18, data.getBacid(), textStyle);
				setCellValue(row, 19, data.getReportVersion(), numberStyle);
	...
	private byte[] generateExcelFromResubData(List<IRRBB_DEPOSITS_Archival_Summary_Entity> dataList, String filename,
			String type) throws Exception {
		if (dataList == null || dataList.isEmpty()) {
			logger.warn("No resub data found for IRRBB_DEPOSITS report.");
			return new byte[0];
		}
	...
	public static class IRRBB_DEPOSITS_Update_Row {
		private Long sno;
		private BigDecimal outstandingBalanceInr;
		private String floatingFixed;
		private String existingBenchmark;
	...
	public void updateAllIrrdepRows(List<IRRBB_DEPOSITS_Update_Row> rows, String type) {
		...
		String summaryTable = "RESUB".equalsIgnoreCase(type) ? "BRRS_IRRBB_DEPOSITS_ARCHIVAL_SUMMARYTABLE"
				: "BRRS_IRRBB_DEPOSITS_SUMMARYTABLE";
		String detailTable = "RESUB".equalsIgnoreCase(type) ? "BRRS_IRRBB_DEPOSITS_ARCHIVAL_DETAILTABLE"
				: "BRRS_IRRBB_DEPOSITS_DETAILTABLE";
		...
		String summarySql = "UPDATE " + summaryTable + " SET " + "FLOATING_FIXED = ?, " + "EXISTING_BENCHMARK = ?, "
				...
				+ "SPREAD_OVER_BENCHMARK = ?, " + "CAP_FLOOR_RATE_OF_INTEREST = ?, " + "REMARKS = ? " + "WHERE SNO = ?";

		String detailSql = "UPDATE " + detailTable + " SET " + "FLOATING_FIXED = ?, " + "EXISTING_BENCHMARK = ?, "
				+ "EXISTING_REPRICING_FREQUENCY = ?, " + "LAST_REPRICING_DATE = ?, " + "NEXT_REPRICING_DATE = ?, "
				+ "SPREAD_OVER_BENCHMARK = ?, " + "CAP_FLOOR_RATE_OF_INTEREST = ?, " + "REMARKS = ? " + "WHERE SNO = ?";

		for (IRRBB_DEPOSITS_Update_Row row : rows) {
			if (row.getSno() == null)
				continue;
			...
			jdbcTemplate.update(summarySql, row.getFloatingFixed(), row.getExistingBenchmark(),
					...
					row.getCapFloorRateOfInterest(), row.getRemarks(), row.getSno());
			...
			if (detailRowsUpdated == 0) {
				String detailSqlFallback = "UPDATE " + detailTable + " SET " + "FLOATING_FIXED = ?, "
						...
						+ "CAP_FLOOR_RATE_OF_INTEREST = ?, " + "REMARKS = ? "
						+ "WHERE REPORT_DATE = (SELECT REPORT_DATE FROM " + summaryTable + " WHERE SNO = ?) "
						+ "AND (ACCOUNT_NUMBER = (SELECT ACCOUNT_NUMBER FROM " + summaryTable + " WHERE SNO = ?) "
						+ " OR CUSTOMER_ID = (SELECT CUSTOMER_ID FROM " + summaryTable + " WHERE SNO = ?))";
				...
				try {
					jdbcTemplate.update(detailSqlFallback, row.getFloatingFixed(), row.getExistingBenchmark(),
							...
							row.getSpreadOverBenchmark(), row.getCapFloorRateOfInterest(), row.getRemarks(),
							row.getSno(), row.getSno(), row.getSno());
				} catch (Exception ex) {
		...
		try {
			reportDateStr = jdbcTemplate.queryForObject(
					"SELECT TO_CHAR(REPORT_DATE, 'DD-MM-YYYY') FROM " + summaryTable + " WHERE SNO = ?", String.class,
					rows.get(0).getSno());
		} catch (Exception e) {
			logger.warn("Could not query REPORT_DATE for SNO {}: {}", rows.get(0).getSno(), e.getMessage());
		}

		// Sequentially execute procedures: FIRST BRRS_IRRBB_DEP_DETAIL_PROCEDURE, THEN
		// BRRS_IRRBB_DEP_SUMMARY_PROCEDURE
		executeIrrdepProcedures(reportDateStr);
	}

	private void executeIrrdepProcedures(String reportDateStr) {
		// 1. First run BRRS_IRRBB_DEP_DETAIL_PROCEDURE
		try {
			logger.info("Executing BRRS_IRRBB_DEP_DETAIL_PROCEDURE with date: {}", reportDateStr);
			if (reportDateStr != null && !reportDateStr.trim().isEmpty()) {
				jdbcTemplate.update("BEGIN BRRS_IRRBB_DEP_DETAIL_PROCEDURE(?); END;", reportDateStr);
			} else {
				logger.warn("Skipping BRRS_IRRBB_DEP_DETAIL_PROCEDURE: no REPORT_DATE resolved.");
			}
			logger.info("BRRS_IRRBB_DEP_DETAIL_PROCEDURE executed successfully.");
		} catch (Exception e) {
			logger.error("Error executing BRRS_IRRBB_DEP_DETAIL_PROCEDURE", e);
			throw new RuntimeException("Detail Procedure Execution Failed: " + e.getMessage(), e);
		}

		// 2. Then run BRRS_IRRBB_DEP_SUMMARY_PROCEDURE
		try {
			logger.info("Executing BRRS_IRRBB_DEP_SUMMARY_PROCEDURE with date: {}", reportDateStr);
			if (reportDateStr != null && !reportDateStr.trim().isEmpty()) {
				jdbcTemplate.update("BEGIN BRRS_IRRBB_DEP_SUMMARY_PROCEDURE(?); END;", reportDateStr);
			} else {
				logger.warn("Skipping BRRS_IRRBB_DEP_SUMMARY_PROCEDURE: no REPORT_DATE resolved.");
			}
			logger.info("BRRS_IRRBB_DEP_SUMMARY_PROCEDURE executed successfully.");
		} catch (Exception e) {
			logger.error("Error executing BRRS_IRRBB_DEP_SUMMARY_PROCEDURE", e);
