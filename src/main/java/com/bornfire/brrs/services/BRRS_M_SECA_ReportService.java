...
public class BRRS_M_SECA_ReportService {
	...
	@Autowired
	BRRS_M_SECA_Manual_Summary_Repo BRRS_M_SECA_Manual_Summary_Repo;

	@Autowired
	BRRS_M_SECA_Archival_Detail_Repo BRRS_M_SECA_Archival_Detail_Repo;

	@Autowired
	BRRS_M_SECA_Manual_Archival_Summary_Repo BRRS_M_SECA_Manual_Archival_Summary_Repo;
	...
	public ModelAndView getM_SECAview(String reportId, String fromdate, String todate, String currency,
		...
		if (type.equals("ARCHIVAL") & version != null) {
			...
			List<M_SECA_Archival_Summary_Entity> T2Master = new ArrayList<M_SECA_Archival_Summary_Entity>();
			System.out.println(version);
			try {
				...
				T1Master = BRRS_M_SECA_Manual_Archival_Summary_Repo.getdatabydateListarchival(dateformat.parse(todate), version);
				T2Master = BRRS_M_SECA_Archival_Summary_Repo.getdatabydateListarchival(dateformat.parse(todate), version);
			...
			List<M_SECA_Summary_Entity> T2Master = new ArrayList<M_SECA_Summary_Entity>();
			try {
				Date d1 = dateformat.parse(todate);
				...
				// ", BRF1_REPORT_ENTITY.class)
				// .setParameter(1, df.parse(todate)).getResultList();
				T1Master = BRRS_M_SECA_Manual_Summary_Repo.getdatabydateList(dateformat.parse(todate));
				T2Master = BRRS_M_SECA_Summary_Repo.getdatabydateList(dateformat.parse(todate));
	...
	public void updateReport1(M_SECA_Manual_Summary_Entity updatedEntity) {
	    ...
	    M_SECA_Manual_Summary_Entity existing = BRRS_M_SECA_Manual_Summary_Repo.findById(updatedEntity.getReport_date())
...
System.out.println("Procedure executed for date: " + oracleDate);
	...
	public byte[] BRRS_M_SECAExcel(String filename, String reportId, String fromdate, String todate, String currency,
		...
		// Fetch data
		List<M_SECA_Manual_Summary_Entity> dataList = BRRS_M_SECA_Manual_Summary_Repo.getdatabydateList(dateformat.parse(todate));
		List<M_SECA_Summary_Entity> dataList1 = BRRS_M_SECA_Summary_Repo.getdatabydateList(dateformat.parse(todate));
		...
		try (InputStream templateInputStream = Files.newInputStream(templatePath);
			...
			if (!dataList.isEmpty() || !dataList1.isEmpty()) {
				for (int i = 0; i < dataList.size(); i++) {
					...
					M_SECA_Summary_Entity record1 = dataList1.get(i);
...
//					}
					...
					if (record1.getR36_BONDS() != null) {
					    cell3.setCellValue(record1.getR36_BONDS().doubleValue());
					    cell3.setCellStyle(numberStyle);
					} else {
					    cell3.setCellValue("");
					    cell3.setCellStyle(textStyle);
					...
					// COLUMN4
					cell4 = row.createCell(3);
					if (record1.getR36_BOBC() != null) {
					    cell4.setCellValue(record1.getR36_BOBC().doubleValue());
					    cell4.setCellStyle(numberStyle);
					} else {
					    cell4.setCellValue("");
					    cell4.setCellStyle(textStyle);
...
List<M_SECA_Detail_Entity> reportData = BRRS_M_SECA_Detail_Repo.getdatabydateList(parsedToDate);

if (reportData != null && !reportData.isEmpty()) {
int rowIndex = 1;
for (M_SECA_Detail_Entity item : reportData) {
...
//	}
	...
	public byte[] getExcelM_SECAARCHIVAL(String filename, String reportId, String fromdate, String todate,
		...
		List<M_SECA_Manual_Archival_Summary_Entity> dataList = BRRS_M_SECA_Manual_Archival_Summary_Repo
				.getdatabydateListarchival(dateformat.parse(todate), version);
		List<M_SECA_Archival_Summary_Entity> dataList1 = BRRS_M_SECA_Archival_Summary_Repo
				.getdatabydateListarchival(dateformat.parse(todate), version);
		...
		try (InputStream templateInputStream = Files.newInputStream(templatePath);
			...
			if (!dataList.isEmpty() || !dataList1.isEmpty()) {
				for (int i = 0; i < dataList.size(); i++) {
					...
					M_SECA_Archival_Summary_Entity record1 = dataList1.get(i);
					System.out.println("rownumber=" + startRow + i);
					Row row = sheet.getRow(startRow + i);
					if (row == null) {
						row = sheet.createRow(startRow + i);
