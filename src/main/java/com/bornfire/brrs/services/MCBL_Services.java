...
public class MCBL_Services {
	...
	@Async
	public void initializeJobStatus(String jobId, MultipartFile file, String userid,
            String username, String reportDate) {

logger.info("Initializing job. jobId={}, userId={}, reportDate={}",
jobId, userid, reportDate);
...
startMCBLUploadAsync(jobId, file, userid, username, reportDate);
	...
	public String startMCBLUploadAsync(String jobId, MultipartFile file,
            String userid, String username, String reportDate) {
...
return "ERROR:" + e.getMessage();
	...
	public String addMCBL(MultipartFile file, String userid, String username, String reportDate) {
	    ...
	    try (InputStream is = file.getInputStream(); Connection conn = dataSource.getConnection()) {
	        ...
	        try (PreparedStatement ps = conn.prepareStatement("SELECT MCBL_HEAD_ACC_NO FROM MCBL WHERE REPORT_DATE = ? AND DEL_FLG = 'N'")) {
	        ...
	        try (PreparedStatement ps = conn.prepareStatement("SELECT MCBL_HEAD_ACC_NO, MAX(VERSION) as MAX_VERSION FROM MCBL WHERE REPORT_DATE = ? GROUP BY MCBL_HEAD_ACC_NO")) {
	        ...
	        String finalMsg = String.format("SUCCESS: Processed %d rows (Skipped %d) in %d ms.", count, skippedRows, duration);
